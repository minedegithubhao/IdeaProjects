...
export default {
  ...
  data() {
    ...
    let checkDuplicateUser = (rule, value, callback) => {
      // 修改时id有值，直接通过不再校验id的唯一性
      if (this.form.id) {
        return callback();
      }
      this.$axios.get(this.$httpUrl + "/user/getUserByNo?no=" + this.form.no).then(res => res.data).then(res=>{
        if (res.code !== 200){
          callback()
        }else {
    ...
    return {
      ...
      rules: {
        no: [
          ...
          {validator: checkDuplicateUser, trigger: 'blur'}
        ],
        name: [
